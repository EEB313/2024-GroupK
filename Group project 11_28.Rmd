...
```{r}
model_2 <- glm(acquired_terr ~ z.density, 
             family = "binomial", 
             data = autumn_juveniles)
summary(model_2)
```
...
```{r}
autumn_juveniles |>
  ggplot(aes(x = z.density, y = acquired_terr)) + 
  geom_point() +
  geom_smooth(method = "glm", 
              method.args = list(family = "binomial")
              )
```
# There is a positive association between z.density and probability of territory acquisition. However, the AIC is rather high perhaps. Need comparision with others factors to see what contribute the most to territory acquisition. 


### Study the effect of z.cones to acquire_terr

```{r}
model_3 <- glm(acquired_terr ~ z.cones, 
             family = "binomial", 
             data = autumn_juveniles)
summary(model_3)
```
...
```{r}
autumn_juveniles |>
  ggplot(aes(x = z.cones, y = acquired_terr)) + 
  geom_point() +
  geom_smooth(method = "glm", 
              method.args = list(family = "binomial")
              )
