...
aic_values <- data.frame(
  Model = c("Full model", "sex", "Std.BD", "Std.growth", "Std.lynx", "Std.hare.fall", "Std.mustelid", "Std.Cleth", "z.density", "z.cones", "z.temp"),
  AIC = c(
    AIC(model_full),
    AIC(model_sex),
    AIC(model_StdBD),
...
## Visualize AIC, and perhaps delta AIC, using bar plots
## calculate the combined effects by multiplying varibales that interact
## Lasso regression (optional)
## Logical presentation, context, methods, and results

##Need revision
### Result for Prediction 2: AIC shows that Standardized birth date (Std.BD) is the biggest factor that effect whether they can acquire a territory
...
```{r}
# Create a new data frame summarizing the count of acquired_terr == 1 by year
yearly_count <- juveniles_data %>%
  group_by(year) %>%
  summarise(count_acquired_terr = sum(acquired_terr == 1))

# Plot for Acquired Territory and Std.lynx over Time
ggplot(juveniles_data, aes(x = year)) +
  geom_line(data = yearly_count, aes(y = count_acquired_terr, color = "acquired_terr"), size = 1) +
  geom_line(aes(y = Std.lynx * 10, color = "Std.lynx"), size = 1) + # Scaling Std.lynx
  labs(title = "Acquired Territory and Std.lynx over Time",
       x = "Year",
       y = "Count of Acquired Territory") +
  scale_color_manual(values = c("acquired_terr" = "blue", "Std.lynx" = "red")) +
  scale_y_continuous(name = "Count of Acquired Territory",
                     sec.axis = sec_axis(~./10, name = "Std.lynx")) + # Secondary axis for Std.lynx
  theme_minimal() +
  theme(legend.title = element_blank()) +
  theme(legend.position = "bottom")

# Plot for Acquired Territory and Std.hare.fall over Time
ggplot(juveniles_data, aes(x = year)) +
  geom_line(data = yearly_count, aes(y = count_acquired_terr, color = "acquired_terr"), size = 1) +
  geom_line(aes(y = Std.hare.fall * 10, color = "Std.hare.fall"), size = 1) + # Scaling Std.hare.fall
  labs(title = "Acquired Territory and Std.hare.fall over Time",
       x = "Year",
       y = "Count of Acquired Territory") +
  scale_color_manual(values = c("acquired_terr" = "blue", "Std.hare.fall" = "red")) +
  scale_y_continuous(name = "Count of Acquired Territory",
                     sec.axis = sec_axis(~./10, name = "Std.hare.fall")) + # Secondary axis for Std.hare.fall
  theme_minimal() +
  theme(legend.title = element_blank()) +
  theme(legend.position = "bottom")

# Plot for Acquired Territory and Std.mustelid over Time
ggplot(juveniles_data, aes(x = year)) +
  geom_line(data = yearly_count, aes(y = count_acquired_terr, color = "acquired_terr"), size = 1) +
  geom_line(aes(y = Std.mustelid * 10, color = "Std.mustelid"), size = 1) + # Scaling Std.mustelid
  labs(title = "Acquired Territory and Std.mustelid over Time",
       x = "Year",
       y = "Count of Acquired Territory") +
  scale_color_manual(values = c("acquired_terr" = "blue", "Std.mustelid" = "red")) +
  scale_y_continuous(name = "Count of Acquired Territory",
                     sec.axis = sec_axis(~./10, name = "Std.mustelid")) + # Secondary axis for Std.mustelid
  theme_minimal() +
  theme(legend.title = element_blank()) +
  theme(legend.position = "bottom")

```
You can ignore this part, I just used chatGPT to *quickly* graph them to see if there is any trend between the number of predators and prey compared to the territory acquisition. 
I just think there are some interesting things to see, like when there is high level of lynx, the territory acquisition was high then drop pretty quickly?
or high peak of hare result in the higher peak of territory acquisition.
However, the number of observations are not the same every year... so prob not a good idea.